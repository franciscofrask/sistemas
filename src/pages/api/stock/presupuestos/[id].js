...
export default async function handler(req, res) {
  ...
  // ✅ DELETE: eliminar presupuesto si está en borrador
  if (req.method === 'DELETE') {
    try {
      await db.query(`CALL EliminarPresupuesto(?)`, [parseInt(id)]);
      return res.status(200).json({ mensaje: 'Presupuesto eliminado correctamente' });
    } catch (error) {
