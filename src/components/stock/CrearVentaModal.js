...
const CrearVentaModal = ({ opened, onClose, onVentaCreada }) => {
  const form = useForm({
    initialValues: {
      id_cliente: '',
      id_vendedor: 2,
  ...
  const handleGuardar = async () => {
    ...
    try {
      const res = await fetch('/api/stock/ventas', {
        method: 'POST',
        headers: {
      ...
      if (!res.ok) {
        ...
        throw new Error(error.error || 'Error al guardar venta');
      }
      ...
      // Callback para actualizar la lista de ventas
      if (onVentaCreada) {
        onVentaCreada();
      }
    ...
    >
      <Stack gap="md">
        <Card shadow="sm" padding="lg" radius="md" withBorder>
          <Title order={4} mb="md">Información de la venta</Title>
          <Grid>
            <Grid.Col span={6}>
              <Select
                readOnly
